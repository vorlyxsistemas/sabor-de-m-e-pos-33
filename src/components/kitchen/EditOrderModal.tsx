...
import { Loader2, Plus, Minus, Trash2, Save, X, AlertCircle } from "lucide-react";
...
export function EditOrderModal({ open, onOpenChange, order, onOrderUpdated }: EditOrderModalProps) {
  ...
  const handleSave = async () => {
    ...
    if (isBlocked) {
      toast({ title: "Não é possível editar pedidos cancelados ou entregues", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      // Update order with new totals and modification tracking
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        {isBlocked ? (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              Não é possível editar pedidos com status "{order.status === "cancelled" ? "Cancelado" : "Entregue"}".
            </AlertDescription>
          </Alert>
        ) : (
          <div className="space-y-4">
            {/* Warning for orders in preparation */}
            {order.status === "preparing" && (
              <Alert>
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  Este pedido já está em preparo. Novos itens adicionados serão marcados como adição posterior.
                </AlertDescription>
              </Alert>
            )}
